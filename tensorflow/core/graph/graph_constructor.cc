...
std::string not_found_nodes[10000];
int not_found_nodes_count = 0;
std::ofstream not_found_count;
std::ofstream myfile;
//end fareed
...
Status GraphConstructor::MakeNode(NodeDef&& node_def, Node** node) {
  ...
  *node = g_->AddNode(std::move(node_def), &status);
  ...
  std::string nd_name = (*node)->def().name();
  std::transform(nd_name.begin(), nd_name.end(), nd_name.begin(),
    [](unsigned char c){ return std::tolower(c); });

  if(mp.find(nd_name) != mp.end()){ 
	(*node)->set_assigned_device_name(mp[nd_name]);
	myfile<<nd_name<<" "<<mp[nd_name]<<"\n";
  }
  else{
	 // if(not_found_nodes_count < 10000)
		//not_found_nodes[not_found_nodes_count] = nd_name;
	    //not_found_nodes_count++;
	}
...
Status GraphConstructor::Convert() {
  ...
  //infile.close();
  //myfile.close();
  //not_found_file.open ("/home/nahmad/not_found.txt");
  //not_found_file<<"fareed";
  //not_found_file.close();
	 
	//not_found_count.open ("/home/nahmad/not_found_count.txt");
  ...
  //fareed
	//myfile.close();
	//int not_found_nodes_limit = std::min(not_found_nodes_count, 10000);
	//not_found_file.open ("/home/nahmad/not_found.txt");
	//not_found_file<<not_found_nodes_count<<"\n";
	//for(int i=0; i< not_found_nodes_limit; i++){
	 //not_found_file<<not_found_nodes[i]<<"\n";
	//}
	//not_found_file.close();
	//not_found_count.close();
	//end fareed
